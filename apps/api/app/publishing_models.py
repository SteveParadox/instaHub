...
class Post(Base,Timestamped):
 __tablename__="posts"
 id:Mapped[uuid.UUID]=mapped_column(primary_key=True,default=uuid.uuid4);workspace_id:Mapped[uuid.UUID]=mapped_column(index=True);media_asset_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("media_assets.id"));caption:Mapped[str]=mapped_column(Text);platform:Mapped[str]=mapped_column(String(30),default="instagram");status:Mapped[str]=mapped_column(String(30),default="draft");published_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True),nullable=True)
