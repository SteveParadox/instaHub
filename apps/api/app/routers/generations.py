...
class Create(BaseModel): workspace_id:uuid.UUID;prompt:str=Field(min_length=3,max_length=4000);style_preset:str="Photorealistic";aspect_ratio:str="1:1";output_count:int=Field(default=1,ge=1,le=4)
...
def assets(workspace_id:uuid.UUID,user:UserProfile=Depends(current_user),db:Session=Depends(get_db)):
 return list(db.scalars(select(MediaAsset).where(MediaAsset.workspace_id==workspace_id).order_by(MediaAsset.created_at.desc()).limit(48)))
