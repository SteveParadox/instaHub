...
class SocialAccount(Base,Timestamped):
 __tablename__="social_accounts";__table_args__=(UniqueConstraint("workspace_id","platform","external_account_id"),)
 id:Mapped[uuid.UUID]=mapped_column(primary_key=True,default=uuid.uuid4);workspace_id:Mapped[uuid.UUID]=mapped_column(index=True);platform:Mapped[str]=mapped_column(String(30),default="instagram");account_name:Mapped[str]=mapped_column(String(255));external_account_id:Mapped[str]=mapped_column(String(255));encrypted_access_token:Mapped[str]=mapped_column(Text);encrypted_refresh_token:Mapped[str|None]=mapped_column(Text,nullable=True);token_expires_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True),nullable=True);metadata_json:Mapped[dict]=mapped_column(JSON,default=dict);is_active:Mapped[bool]=mapped_column(Boolean,default=True)
